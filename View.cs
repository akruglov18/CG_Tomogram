    ...
    class View
    {
           ...
           for (int x_coord = 0; x_coord < Bin.X - 1; x_coord++) 
            {
                for (int y_coord = 0; y_coord < Bin.Y -1; y_coord++)
                {
                    for (int i = 0; i < 2; i++)
                    {
                        value = Bin.array[x_coord+i  + (y_coord) * Bin.X
                                    + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        GL.Vertex2(x_coord+i , (y_coord));

                        value = Bin.array[x_coord+i  + (y_coord + 1) * Bin.X
                                            + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        GL.Vertex2(x_coord+i , (y_coord + 1));
                    }
        ...
        public void DrawTexture()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                GL.TexCoord2((float)i, 0f);
                GL.Vertex2(i * Bin.X, 0);

                GL.TexCoord2(i, 1f);
                GL.Vertex2(i * Bin.X, Bin.Y);
            }
